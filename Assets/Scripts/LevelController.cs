...
public class LevelController : MonoBehaviour {
	...
	private int minMultiplicand = 2, maxMultiplicand = 5;		//the multiplicand is the second number in a multiplication problem.

	private int divMin = 2, divMax = 11; 		// The minimum and mazimum values for the division problem variables
	private int maxDenom = 5, minDenom = 2;
...
//		Debug.Log ("LEVEL STARTED STATUS: " + levelStarted);
		...
		switch (levelSwitch) 
		{
			case 1: // Addition
			{
				...
				{		
					...
					Debug.Log ("SCALING: addMin = " + addMin);
					Debug.Log ("SCALING: addMax = " + (addMax - 1));
				}
...
//				subMax = 12;
				...
				//Widens the range between the generated numbers every 5 (addition) questions answered
				if (((level - 2) % 5 == 0) && levelStarted == false) 
				{		
					subMin += (subtractionAnswered / 2);
					subMax += (subtractionAnswered * 2);
					Debug.Log ("SCALING: subMin = " + subMin);
					Debug.Log ("SCALING: subMax = " + (subMax - 1));
				}
...
//							subtractionAnswered++;
			...
			case 3: // Multiplication
			{
				...
				{			
					...
					maxMultiplicand += (multiplicationAnswered / 5);

					Debug.Log ("SCALING: mulMin = " + mulMin);
					Debug.Log ("SCALING: mulMax = " + (mulMax - 1));
					Debug.Log ("SCALING: maxMultiplicand = " + (maxMultiplicand - 1));
				...
				while (answerValue == 0) 
				{
					...
					num2 = Random.Range (minMultiplicand, maxMultiplicand);			

					answerValue = num1 * num2;
//					answerRange = new Vector2 ((mulMin * mulMin), (mulMax * mulMax));
					answerRange = new Vector2 ((mulMin * minMultiplicand), (mulMax * maxMultiplicand));
					gc.UpdateProblem (num1 + " * " + num2 + " =");
				}
...
//				int minDenom = 2;
				...
				if (((level - 4) % 5 == 0) && levelStarted == false) 
				{		
					divMin += (divisionAnswered / 5);
					divMax += (divisionAnswered / 5);
					maxDenom += (divisionAnswered / 5);
					Debug.Log ("SCALING: divMin = " + divMin);
					Debug.Log ("SCALING: divMax = " + (divMax - 1));
					Debug.Log ("SCALING: maxDenom = " + (maxDenom - 1));
				}
